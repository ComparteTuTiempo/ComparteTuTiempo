...
export default function ConversationView() {
  ...
  useEffect(() => {
    ...
    .then(res => {
      const otro = res.data.participantes?.find(p => p.correo !== user.correo);
      setOtherUser(otro || null);
    })
  ...
  return (
    <div style={styles.container}>
      ...
      <h3 style={styles.header}>
        {otherUser ? `Chat con ${otherUser.nombre || otherUser.correo}` : "Chat"}
      </h3>
            ...
            <div key={m.id || idx} style={styles.messageWrapper(isMe)}>
              <div style={styles.messageBubble(isMe)}>
                <div style={styles.senderName}>
                  {isMe ? "Tú" : m.remitente?.nombre || m.remitente?.correo || "Usuario"}
                </div>
...
const styles = {
  ...
  header: { textAlign: "center", fontSize: "18px", marginBottom: "10px" },
  messagesContainer: {
    height: 400,
    overflowY: "auto",
